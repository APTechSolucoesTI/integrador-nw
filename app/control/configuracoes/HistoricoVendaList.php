...
class HistoricoVendaList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'form_HistoricoVendaList';
    ...
    public function __construct($param = null)
    {
        ...
        $this->form->setFormTitle("Histórico de Vendas");
        $this->limit = 20;
        ...
        $meta_id = new BDBSelectCheck('meta_id', 'integrador', 'Meta', 'id', '{mes_ano}','id asc' , $criteria_meta_id );
        ...
        $grupo_cliente_id = new TDBCombo('grupo_cliente_id', 'integrador', 'ApGrupoCliente', 'id', '{descricao}','id asc' , $criteria_grupo_cliente_id );
        $cod_clifor = new TEntry('cod_clifor');
        $nro = new TEntry('nro');
        $agente = new TEntry('agente');
        ...
        $row1 = $this->form->addFields([new TLabel("Mês:", null, '14px', null, '100%'),$meta_id],[new TLabel("Emissão:", null, '14px', null, '100%'),$data_emissao],[new TLabel("Consultor(a):", null, '14px', null, '100%'),$repres_id],[new TLabel("Grupo:", null, '14px', null, '100%'),$grupo_cliente_id]);
        $row1->layout = [' col-sm-3',' col-sm-3',' col-sm-3',' col-sm-3'];

        $row2 = $this->form->addFields([new TLabel("Código:", null, '14px', null, '100%'),$cod_clifor],[new TLabel("Número:", null, '14px', null, '100%'),$nro],[new TLabel("Reativação:", null, '14px', null, '100%'),$agente]);
        ...
        $column_meta_mes_ano = new TDataGridColumn('meta->mes_ano', "Meta", 'left');
        $column_data_emissao_transformed = new TDataGridColumn('data_emissao', "Emissão", 'left');
        $column_nro = new TDataGridColumn('nro', "Número", 'left');
        $column_cod_clifor = new TDataGridColumn('cod_clifor', "Código", 'left');
        $column_razao = new TDataGridColumn('razao', "Cliente", 'left');
        $column_agente = new TDataGridColumn('agente', "Reativação", 'left');
        $column_repres_id = new TDataGridColumn('repres_id', "Consultor(a)", 'left');
        $column_grupo_cliente_id = new TDataGridColumn('grupo_cliente_id', "Grupo", 'left');
        ...
        $this->datagrid->addColumn($column_agente);
        $this->datagrid->addColumn($column_repres_id);
        $this->datagrid->addColumn($column_grupo_cliente_id);
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['HistoricoVendaList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['HistoricoVendaList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['HistoricoVendaList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['HistoricoVendaList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Configurações","Histórico de Vendas"]));
        }
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->meta_id) AND ( (is_scalar($data->meta_id) AND $data->meta_id !== '') OR (is_array($data->meta_id) AND (!empty($data->meta_id)) )) )
        {
            ...
            $filters[] = new TFilter('meta_id', 'in', $data->meta_id);// create the filter 
        }
        ...
        if (isset($data->agente) AND ( (is_scalar($data->agente) AND $data->agente !== '') OR (is_array($data->agente) AND (!empty($data->agente)) )) )
        {

            $filters[] = new TFilter('agente', 'like', "%{$data->agente}%");// create the filter 
        }
