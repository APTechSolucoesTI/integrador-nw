...
class PlanejamentoImportList extends TPage
{
    ...
    private static $activeRecord = 'PlanejamentoImportDias';
    private static $primaryKey = 'id';
    private static $formName = 'formList_PlanejamentoImportDias';
    private $limit = 20;
    ...
    public function __construct($param = null)
    {
        ...
        $column_planejamento_import_id = new TDataGridColumn('planejamento_import_id', "planejamento_import_id", 'left');
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Configurações","PlanejamentoImportList"]));
        }
        $container->add($panel);
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for PlanejamentoImportDias
...
*/              $objects = $repository->load($criteria, FALSE);
                ...
                if ($objects)
                {
                    foreach ($objects as $object)
                    {
                        ...
                        if ($object->valido === 'S')
                        {
                            $check->setValue([$object->id => $object->id]);
                        }

                        $check->setChangeAction(new TAction([__CLASS__, 'onToggleValido']));
    ...
    public static function manageRow($id, $param = [])
    {
        ...
        $object = new PlanejamentoImportDias($id);
    ...
    try{
            ...
            $id = empty($param['key']) ? $valueOn : $param['key'];

            $dia = new PlanejamentoImportDias($id);

            if (!empty($param['builder_datagrid_check']) && in_array($id, $param['builder_datagrid_check']))
            {
                $dia->valido = 'S';
            }
            else
