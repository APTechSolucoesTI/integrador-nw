...
class STVincularTabelaPrecoWindowList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $column_cod_tabelapreco = new TDataGridColumn('cod_tabelapreco', "Cód. Tabela de Preço", 'center');
        ...
        $button_vincular->setAction(new TAction(['STVincularTabelaPrecoWindowList', 'onVincularPreco']), "Vincular");
    ...
    public function onVincularPreco($param = null) 
    {
        try 
        {
            ...
            $mini_meta_id = TSession::getValue('mini_meta_id');
            $idsSelecionados = TSession::getValue('STVincularTabelaPrecoWindowListbuilder_datagrid_check');

            if (!$mini_meta_id) {
                throw new Exception("ST não informado.");
            }
            ...
            foreach ($tabelas as $tabela) {
                ...
                // Procura o vínculo
                $registro = MinimetaTabelaPreco::where('mini_meta_id', '=', $mini_meta_id)
                ...
                if ($registro) {
                    ...
                    $registro = new MinimetaTabelaPreco;
                    $registro->mini_meta_id = $mini_meta_id;
    ...
    public function onShow($param = null)
    {
        ...
        if (isset($param['mini_meta_id'])) {
            TSession::setValue('mini_meta_id', $param['mini_meta_id']);
        }

        $mini_meta_id = $param['mini_meta_id'] ?? TSession::getValue('mini_meta_id');
        $idsMarcados = [];

        if ($mini_meta_id) {
            TTransaction::open('integrador');

            $registros = MinimetaTabelaPreco::where('mini_meta_id', '=', $mini_meta_id)
                                            ->where('fazparte', '=', 'S')
                                            ->load();
            ...
            TSession::setValue('STVincularTabelaPrecoWindowListbuilder_datagrid_check', array_combine($idsMarcados, $idsMarcados));
        } else {
            TSession::setValue('STVincularTabelaPrecoWindowListbuilder_datagrid_check', []);
        }
